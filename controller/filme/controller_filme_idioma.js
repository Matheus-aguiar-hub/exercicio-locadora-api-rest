...
const atualizarFilmeIdioma = async function(filmeIdioma, id, contentType){
    ...
    try{
            ...
            if(resultBuscarID.status){
                let validar = await validarDados(filmeIdioma, contentType)
...
const buscarFilmeIdIdioma = async function(idIdioma){
   ...
   let message = JSON.parse(JSON.stringify(config_message))
       ...
       try {
           ...
           let result = await filmeIdiomaDAO.selectFilmesByIdIdioma(idIdioma)
       ...
       try {
           ...
           if(result){
               if(result.length > 0){
                   message.DEFAULT_MESSAGE.status          = message.SUCESS_RESPONSE.status
                   message.DEFAULT_MESSAGE.status_code     = message.SUCESS_RESPONSE.status_code
                   message.DEFAULT_MESSAGE.response.filme_idioma = result[0]
...
const buscarFilmeIdioma = async function(id){
    ...
    let message = JSON.parse(JSON.stringify(config_message))
        ...
        try {
            ...
            let result = await filme_idiomaDAO.selectAllFilmeIdioma(id)
...
const excluirFilmeIdioma = async function(id){
    ...
    try{
        ...
        if(resultBuscarID.status){
            let result = await filme_idiomaDAO.deleteFilmeIdioma(id)
